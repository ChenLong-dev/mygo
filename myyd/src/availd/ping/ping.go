...
func NewIpsPinger(addr string, ips map[string]bool) (*Pinger, error) {
	ipaddr, err := net.ResolveIPAddr("ip", addr)
	if err != nil {
		return nil, err
	}
...
func (p *Pinger) recvICMP(
	...
	for {
		...
		default:
			...
			if p.ipv4 {
				//var cm *ipv4.ControlMessage
				n, cm, ipv4Addr, err = conn.IPv4PacketConn().ReadFrom(bytes)
				if cm != nil {
					ttl = cm.TTL
				}
				//mlog.Debugf("ping ip:%s, src ip:%s, dst ip:%s, ipv4Addr:%s, cm:%v", p.addr, string(cm.Src), string(cm.Dst), ipv4Addr.String(), cm)
			} else {
			...
			select {
			case recv <- &packet{bytes: bytes, nbytes: n, ttl: ttl, ip: ipv4Addr.String()}:
				if ipv4Addr.String() != "127.0.0.1" && ipv4Addr.String() != "" {
					lock.Lock()
					p.ips[ipv4Addr.String()] = true
					lock.Unlock()
				}
			default:
				//mlog.Debugf("is full")
