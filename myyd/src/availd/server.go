...
func handleCheck(mctx *mcom.MContext, msrv *mcom.MServer, data []byte) (rdata []byte, rerr error) {
	...
	defer func() {
		...
		vt2 := time.Now()
		mlog.Infof("x=x=x=x [vt1:%v] [vt2:%v] [exp:%v]\n", vt1, vt2, vt2.Sub(vt1))
	}()
...
func handlePing(mctx *mcom.MContext, msrv *mcom.MServer, data []byte) (rdata []byte, rerr error) {
	mctx.Tracef("len(data)=%d, data:%v", len(data), data)
	defer func() {
		if rerr != nil {
			mlog.Warnf("len(rdata)=%d,rerr=%v", len(rdata), rerr)
		} else {
	...
	startTime := time.Now()
	mlog.Debugf("startTime=%v", startTime)
	...
	results, cherr := CheckPing(checkReq.Tasks, time.Duration(checkReq.Timeout)*time.Second)
	if cherr != nil {
		return nil, fmt.Errorf("Check error:%v", cherr)
	}
	...
	mlog.Infof("[results.len=%d], [exp=%v]", len(results), endTime.Sub(startTime))
	...
	//mlog.Debugf("handle ping rsp: %s", rspData)
	gzRspData, gzCerr := mutils.GZipCompress(rspData)
	if gzCerr != nil {
		return nil, fmt.Errorf("GZipCompress error:%v", gzCerr)
	}
...
func handle(mctx *mcom.MContext, msrv *mcom.MServer, data []byte) {
	...
	case mctx.OpCode() == scom.OPCODE_PING_REQ:
		rdata, rerr := handlePing(mctx, msrv, data)
		mlog.Debugf("handle ping1111")
		msrv.Reply(mctx, rerr, rdata)
		mlog.Debugf("handle ping2222")
	default:
